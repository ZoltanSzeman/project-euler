...
biglist= [list1,list2,list3,list4,list5,list6,list7,list8,list9,list10,list11,list12,list13,list14,list15,list16,list17,list18,list19,list20]
...
while list_count != 20:
  if count == 20:
    ...
    list_count += 1
  try:
    sor = biglist[list_count][count] * biglist[list_count][count+1] * biglist[list_count][count+2] * biglist[list_count][count+3]
    if sor > result:
      result = sor
  except:
    pass
  try:
    oszlop = biglist[list_count][count] * biglist[list_count+1][count] * biglist[list_count+2][count] * biglist[list_count+3][count]
    if oszlop > result:
      result = oszlop
  except:
    pass
  try:
    atlo = biglist[list_count][count] * biglist[list_count+1][count+1] * biglist[list_count+2][count+2] * biglist[list_count+3][count+3]
    if atlo > result:
      result = atlo
  except:
    pass
  try:
    atlo2 = biglist[list_count][count] * biglist[list_count-1][count+1] * biglist[list_count-2][count+2] * biglist[list_count-3][count+3]
    if atlo2 > result:
      result = atlo2
  except:
    pass
